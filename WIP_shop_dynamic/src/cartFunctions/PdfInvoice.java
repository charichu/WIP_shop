...
public class PdfInvoice {
    ...
    public void print(ArrayList<Course> bookingList, HashMap<Integer, Integer> cart, double sum, int orderID, HttpServletRequest request){
    ...
    private void createPDF(String filename, ArrayList<Course> bookingList, HashMap<Integer, Integer> cart, double sum, int orderID, HttpServletRequest request) throws DocumentException, MalformedURLException, IOException{
        ...
        document.add(paragraph1);
          ...
          Image img = Image.getInstance("WebContent/img/logo.png");
        ...
        ResultSet rs;
		try {
			rs = DBFunctions.Execute("select * from user, addresses, orders where  orders.orderID='"+orderID+"' and user.userID=orders.userID and addresses.addressID=users.addressID;");
			if(rs.next()){
